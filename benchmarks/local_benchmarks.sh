...
run_local_benchmark() {
    ...
    if [ "$implementation" == "rust" ]; then
        ../core/target/release/compression-cli compress "$input" "$output" --algorithm $algorithm
    else
    ...
    start_time=$(date +%s.%N)
    if [ "$implementation" == "rust" ]; then
        ../core/target/release/compression-cli decompress "$output" "$output.decompressed" --algorithm $algorithm
    else
